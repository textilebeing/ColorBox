...
class ColorBox():
    ...
    def __init__(self, WIN_H = 500, WIN_W = 500, WIN_TITLE = "ColorBox v2"):
        ...
        '''
        if (WIN_W or WIN_H) < 500: self.WIN_W = self.WIN_H = 500
        else:
            self.WIN_H = WIN_H
            self.WIN_W = WIN_W
        ...
        self.BOTTOM = 50
        self.COLORS = [ 'black', 'red', 'green', 'blue','white', 'beige',
                       'blue violet', 'dark olive green' ,'dark orange' ,
                       'deep pink' ,'forest green','lavender', 'khaki',
                       'rebecca purple', 'plum' ,'peru', 'powder blue']
    ...
    def _getEntryNum(self):
        ...
        try:
            ...
            self.click_info.setText("Error, Please Enter a Number")
    ...
    def _GUI_setup(self):
        ...
        self.win = GraphWin(self.WIN_TITLE, self.WIN_W,\
                            self.WIN_H + self.BOTTOM)
        ...
        self.bottom = Rectangle(Point(0, self.WIN_H ), \
                                Point(self.WIN_W, self.WIN_H+50))
        self.bottom.setFill(color_rgb(146, 131, 116))
        self.bottom.draw(self.win)
        

        self.btn_Gen = btn_create(self.win, 0, self.WIN_H, 
                                  self.WIN_W*0.33, 50, "GENERATE") # Gen btn

        self.btn_Quit = btn_create(self.win, self.WIN_W*.66, self.WIN_H, 
                                   self.WIN_W*0.34, 50, "QUIT") #Reset btn
    ...
    def circ_gen(self):
        ...
        x1 = random.randint(0, self.WIN_W) # Defining random top left start point
        y1 = random.randint(0, self.WIN_H-30) # top right anywhere from 0 to y2
    ...
    def _color_set_and_fill(self, shape):
        if self.palette == -1: color = self._color_gen()
        else: color = self._palette(self.palette)
    ...
    def oval_gen(self):
        x1 = random.randint(0, self.WIN_W) # Defining random top left start point
        y1 = random.randint(0, self.WIN_H) # bottom left anywhere from 0 to y2
        y2 = random.randint(y1, self.WIN_H) # Defining bottom right end point
        x2 = random.randint(x1, self.WIN_W) # top right anywhere from top left to end
        
        if (abs(x1 - x2)) < 100: x1 -= 20
        if (abs(y1 - y2)) < 100: y1 -= 20
    ...
    def poly_gen(self):
        ...
        in the window.
        '''
        # BLOCK OF COORDINATE GENERATORS
        p1x = random.randint(0, self.WIN_W)
        p1y = random.randint(0, self.WIN_H)
        p2x = random.randint(0, self.WIN_W)
        p2y = random.randint(0, self.WIN_H)
        p3x = random.randint(0, self.WIN_W)
        p3y = random.randint(0, self.WIN_H)
        p4x = random.randint(0, self.WIN_W)
        p4y = random.randint(0, self.WIN_H)
    ...
    def _random_poly_point_generator(self, n):
        self.px = []
        self.py = []
        for x in range(n):
            self.px.append(random.randint(0, self.WIN_W))
            self.py.append(random.randint(0, self.WIN_H))
